...
<div class="content" id="select-house-content">
	...
	<a href="<%=request.getContextPath()%>/yjfk.action?url=<%=java.net.URLEncoder.encode(sbi.xwb.weixin.commons.filemonitor.PropertiesReader.read("server")+request.getContextPath()+"/admin/lc/start.action?flowSign="+request.getParameter("flowSign")+"&lm=yd","UTF-8") %>"></a>
	</div>
    <!--意见反馈-->
...
function getBuildInfo(){
	...
	$(function(){
	  ...
	  searchUrl = '${pageContext.request.contextPath}/admin/lc/search.action';
      getBuildInfo();
 });
</script>
