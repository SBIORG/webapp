...
<body>
  ...
  <div class="content">
      ...
      <div class="feedback_btn">
        <a href="<%=request.getContextPath()%>/yjfk.action?url=<%=java.net.URLEncoder.encode(sbi.xwb.weixin.commons.filemonitor.PropertiesReader.read("server")+request.getContextPath()+"/admin/lc/list.action?lm=yw","UTF-8") %>"></a>
      </div>
      <!--意见反馈-->
    <!--业务-->
    <div class="phone_work" id="wrapper">
        <div id="scroller">
         ...
         <c:when test="${fn:length(flowBusPojos)!=0 }">
            ...
            <a class="all_link" href="${pageContext.request.contextPath}/admin/lc/view.action?flowId=${bus.flow_id}">
                <div class="each_box">
...
function pullUpAction () { //上拉加载更多内容  动态发送ajax请求获取内容
	//setTimeout(function () {	// <-- Simulate network congestion, remove setTimeout from production!
		...
		el = document.getElementById('thelist');
		flag=flag+1;
		showUrl="${pageContext.request.contextPath}/admin/lc/view.action?flowId=";
		if(isLoadAjax==true){
			 $.ajax({
					type: "GET",
					url: "${pageContext.request.contextPath}/admin/lc/ajaxList.action?page="+flag,
					dataType: "json",
