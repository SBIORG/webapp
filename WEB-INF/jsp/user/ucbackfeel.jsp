...
<body>
  ...
  <div class="content">
      ...
      <div class="feedback_btn">
        <a href="<%=request.getContextPath()%>/yjfk.action?url=<%=java.net.URLEncoder.encode(sbi.xwb.weixin.commons.filemonitor.PropertiesReader.read("server")+request.getContextPath()+"/user/ucbackfeel.action?lm=tf","UTF-8") %>"></a>
      </div>
      <!--意见反馈-->
    <!--头部页卡-->
    <div class="center_top_tab">
       <div class="center_tab_nav clearfix">
         ...
         <c:if test="${userInfo.lessee_type!=2}">
           <li class="tab_left normal actived"><a href="${pageContext.request.contextPath}/user/ucbackfeel.action?lm=tf">租赁退费</a></li>
         </c:if>
           <li class="tab_right normal "><a href="${pageContext.request.contextPath}/user/ucbackfeep.action?lm=tf">物业退费</a></li><!--页卡选中：class：actived-->
         </ul>
       </div>
    </div>
    <!--头部页卡-->
    ...
    <div class="center_phone_pages">
      <div class="inner_content">
        ...
        <!--每一条退费信息-->
         <c:forEach var='lbackfee' items='${llist}' varStatus='status'>
        <a class="all_link">
            <div class="each_box">
              <h1 class="each_h1 clearfix">
                <span class=" fl">${lbackfee.fee_type }(<span class="change_color">${lbackfee.status }</span>)</span>
                <span class="fr each_time change_color">${lbackfee.throw_fee }</span><!--红色字加class：change_color-->
              </h1>
              <div class="clearfix each_info">
                 <span class="fl info_text_building all_link_span">${lbackfee.house_no}</span>
                 <span class="fr each_info_tuifei">${lbackfee.cont }</span>
                 <span></span>
              </div>
            </div>
        </a>
