...
<body>
  ...
  <div class="content">
      ...
      <div class="feedback_btn">
        <a href="<%=request.getContextPath()%>/yjfk.action?url=<%=java.net.URLEncoder.encode(sbi.xwb.weixin.commons.filemonitor.PropertiesReader.read("server")+request.getContextPath()+"/user/uccontplist.action?lm=ht","UTF-8") %>"></a>
      </div>
      <!--意见反馈-->
    <!--头部页卡-->
       ...
       <div class="center_tab_nav clearfix">
         <ul>
         	<c:if test="${userInfo.lessee_type!=2}">
          		 <li class="tab_left normal"><a href="${pageContext.request.contextPath}/user/uccontllist.action?lm=ht">租赁合同</a></li>
           </c:if>
           <li class="tab_right normal actived"><a href="${pageContext.request.contextPath}/user/uccontplist.action?lm=ht">物业合同</a></li><!--页卡选中：class：actived-->
         </ul>
       </div>
    </div>
    <!--头部页卡-->
    ...
    <div class="center_phone_pages">
      <div class="inner_content">
        <!--每一条合同信息-->
          <c:forEach var='property' items='${plist}' varStatus='status'>
        <a href="${pageContext.request.contextPath}/user/uccontpview.action?id=${property.id }&lm=ht" class="all_link">
            <div class="each_box">
              <h1 class="each_h1 clearfix">
                <span class=" fl">${property.cont_no }</span>
                <span class="fr each_time">${property.status}</span><!--红色字加class：change_color-->
              </h1>
              <div class="clearfix each_info">
                 <span class="fl info_text all_link_span">${property.allHouse}</span>
                 <span class="fr each_time">${property.date}</span>
              </div>
            </div>
        </a>
        </c:forEach>
