...
<body>
  ...
  <div class="content">
      ...
      <div class="feedback_btn">
        <a href="<%=request.getContextPath()%>/yjfk.action?url=<%=java.net.URLEncoder.encode(sbi.xwb.weixin.commons.filemonitor.PropertiesReader.read("server")+request.getContextPath()+"/user/ucpayfeel.action?lm=jf","UTF-8") %>"></a>
      </div>
      <!--意见反馈-->
    <!--头部页卡-->
    ...
    <div class="center_phone_pages phone_jiaofei">
      <div class="inner_content">
        <!--每一条合同信息-->
         <c:forEach var='lpayfee' items='${llist}' varStatus='status'>
        <div class="each_box">
          <h1 class="each_h1 clearfix">
            <span class=" fl">${lpayfee.fee_type }</span>
            <em class="fr each_choose normal" id="${lpayfee.id }"></em><!--选中：actived-->
            <span class="fr each_time change_color">${lpayfee.mone }</span><!--红色字加class：change_color-->
          </h1>
          <div class="clearfix each_info">
             <span class="fl info_text">
               <a>${lpayfee.house_no }</a>
             </span>
             <span class="fr each_time">${lpayfee.time_s }</span>
          </div>
        </div>
        </c:forEach>
        <!--每一条合同信息-->
